...
"\n\t--bam-rmdup  [path to bam-rewrap]\t\tPath to bam-rmdup from biohazard".
...
my $bamrmdup = "bam-rmdup";

$gmapper    = which($gmapper);
$samtools   = which($samtools);
$bamrmdup  = which($bamrmdup);
#
#if(defined $gmapper){
...
#
#if(defined $bamrmdup){
#  print "bam-rmdup used ".$bamrmdup."\n";
#}else{
#  die "Please install bam-rmdup from https://github.com/udo-stenzel/biohazard";
#}
#
#
#
...
usage() if ( @ARGV < 1 or
	     ! GetOptions('help|?' => \$help, 'gmapper=s' => \$gmapperCMDL, 'samtools=s' => \$samtoolsCMDL, 'bam-rmdup=s' => \$bamrmdupCMDL,'threads=i' => \$threads )
          or defined $help );
...
if($bamrmdupCMDL eq ""){
  ...
  if(-e $bamrmdupCMDL){
    $bamrmdup =  $bamrmdupCMDL;
  }else{
    die "Please specify an existing path for bamrmdup";
  }
}
