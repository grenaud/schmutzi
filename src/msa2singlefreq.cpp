...
int main (int argc, char *argv[]) {
    ...
    const string usage=string("\nThis program takes an multiple sequence alignment (msa) and outputs the allele frequency for all the sequences \n\n\t"+
			      string(argv[0])+                        
	...
	for(unsigned int i=0;i<ids.size();i++){
		...
		if(seq[i][j] == 'N' ||
		   ...
		   isAmbiguousUIPAC(seq[i][j])){
		    ...
		    nucCount[ baseResolved2int(seq[i][j]) ]++;
		    foundOneDefined=true;
		}
	    }
	}
	double sumCount = 
	...
	if(!foundOneDefined)
	    ...
	    cout<<indexRef
		<<"\t"<<  ( double( nucCount[ 0 ] )/sumCount )
		<<"\t"<<  ( double( nucCount[ 1 ] )/sumCount ) 
		<<"\t"<<  ( double( nucCount[ 2 ] )/sumCount )
		<<"\t"<<  ( double( nucCount[ 3 ] )/sumCount )
		<<endl;
